...
For defining an alias in the script use an assignment `=` in a directive:
...
*graffl* supports __namespaces__.
You can define a __namespace prefix__ using the assignment directive with an `=` sign at the beginning of your file:
...
If you use *SHACL* as well, there are naming conflicts, especially for `hasValue`. In that case
you can disambiguate with namespaces:
